...
<html>
<head>
    <title> Recibos Nomina PDF</title>
...
<body>
	...
	@foreach($nominaconcepts as $employee)
		...
		<div class="text-center">
			<h4> RECIBO DE NÓMINA ESPECIAL </h4>
		</div>
		...
		<table class="table-custom">
			...
			<tr> 
				<td> Tipo de comprobante </td> <td  class="text-right"> {{ $nomina->tipo_nomina }} / {{ $nomina->id }} </td> 
			...
			<tr> 
				...
				<td> Fecha inicial de pago </td> <td  class="text-right"> {{ \Carbon\Carbon::parse($nomina->fechaInicio)->format('d-m-Y') }} </td>
			</tr>
			@php
			...
			<tr> 
				...
				<td> Fecha final de pago </td> <td  class="text-right"> {{ \Carbon\Carbon::parse($nomina->fechaFin)->format('d-m-Y') }} </td>
			</tr>
			<tr> 
				<td> Inicio de la relación laboral </td> <td  class="text-right"> {{ \Carbon\Carbon::parse($employee->fechaIngreso)->format('d-m-Y') }} </td> 
				<td> Número de días pagados </td> <td  class="text-right"> {{ $nomina->diasPagados }} </td>
			</tr>
		...
		@php
			$percepciones = $employee->getNominaPercepciones($nomina->id, $employee->expediente); 
			$deducciones = $employee->getNominaDeducciones($nomina->id, $employee->expediente);
			// Contar el número de percepciones del empleado
			$numPercepciones = $percepciones->count();			
		@endphp
		...
		<!-- Condicionar el número de saltos de línea según el número de percepciones -->
		@if($numPercepciones > 2)
			<br><br>
		@else
			<br><br>
			<br><br>
